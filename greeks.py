...
for K, opPrice in calls:
    try:
        ...
        if np.isnan(iv) == False:
            ...
            ax.scatter(K, iv, color='limegreen', s=5)
            
            dS = 0.01*S
            delta_tree1 = Tree(S+dS, K, r, q, t, optype='call', steps=50)
            delta_tree2 = Tree(S-dS, K, r, q, t, optype='call', steps=50)
            delta_tree1.params(iv)
            delta_tree2.params(iv)
            delta = (delta_tree1.optionTree() - delta_tree2.optionTree())/(2.0*dS)
            Delta.scatter(K, delta, color='limegreen', s=5)
            Delta.set_title("Delta")

            gamma_tree1 = Tree(S-dS, K, r, q, t, optype='call', steps=50)
            gamma_tree2 = Tree(S, K, r, q, t, optype='call', steps=50)
            gamma_tree3 = Tree(S+dS, K, r, q, t, optype='call', steps=50)
            gamma_tree1.params(iv)
            gamma_tree2.params(iv)
            gamma_tree3.params(iv)
            gamma = (gamma_tree3.optionTree() - 2.0*gamma_tree2.optionTree() + gamma_tree1.optionTree())/pow(dS, 2)
            Gamma.scatter(K, gamma, color='limegreen', s=5)
            Gamma.set_title("Gamma")

            dT = 1.0/365.0
            theta_tree1 = Tree(S, K, r, q, t+dT, optype='call', steps=50)
            theta_tree2 = Tree(S, K, r, q, t, optype='call', steps=50)
            theta_tree1.params(iv)
            theta_tree2.params(iv)
            theta = -(theta_tree1.optionTree() - theta_tree2.optionTree())/dT
            Theta.scatter(K, theta, color='limegreen', s=5)
            Theta.set_title("Theta")

            dV = 0.01
            vega_tree1 = Tree(S, K, r, q, t, optype='call', steps=50)
            vega_tree2 = Tree(S, K, r, q, t, optype='call', steps=50)
            vega_tree1.params(iv-dV)
            vega_tree2.params(iv+dV)
            vega = (vega_tree2.optionTree() - vega_tree1.optionTree())/(2.0*dV)
            vega /= 100
            Vega.scatter(K, vega, color='limegreen', s=5)
            Vega.set_title("Vega")

            dR = 0.01
            rho_tree1 = Tree(S, K, r+dR, q, t, optype='call', steps=50)
            rho_tree2 = Tree(S, K, r-dR, q, t, optype='call', steps=50)
            rho_tree1.params(iv)
            rho_tree2.params(iv)
            rho = (rho_tree1.optionTree() - rho_tree2.optionTree())/(2.0*dR)
            rho /= 100
            Rho.scatter(K, rho, color='limegreen', s=5)
            Rho.set_title("Rho")
...
for K, opPrice in puts:
    try:
        ...
        iv = tree.impliedVol(opPrice)
        if np.isnan(iv) == False:
            ax.scatter(K, iv, color='red', s=5)
            
            dS = 0.01*S
            delta_tree1 = Tree(S+dS, K, r, q, t, optype='put', steps=50)
            delta_tree2 = Tree(S-dS, K, r, q, t, optype='put', steps=50)
            delta_tree1.params(iv)
            delta_tree2.params(iv)
            delta = (delta_tree1.optionTree() - delta_tree2.optionTree())/(2.0*dS)
            Delta.scatter(K, delta, color='red', s=5)

            gamma_tree1 = Tree(S-dS, K, r, q, t, optype='put', steps=50)
            gamma_tree2 = Tree(S, K, r, q, t, optype='put', steps=50)
            gamma_tree3 = Tree(S+dS, K, r, q, t, optype='put', steps=50)
            gamma_tree1.params(iv)
            gamma_tree2.params(iv)
            gamma_tree3.params(iv)
            gamma = (gamma_tree3.optionTree() - 2.0*gamma_tree2.optionTree() + gamma_tree1.optionTree())/pow(dS, 2)
            Gamma.scatter(K, gamma, color='red', s=5)

            dT = 1.0/365.0
            theta_tree1 = Tree(S, K, r, q, t+dT, optype='put', steps=50)
            theta_tree2 = Tree(S, K, r, q, t, optype='put', steps=50)
            theta_tree1.params(iv)
            theta_tree2.params(iv)
            theta = -(theta_tree1.optionTree() - theta_tree2.optionTree())/dT
            Theta.scatter(K, theta, color='red', s=5)

            dV = 0.01
            vega_tree1 = Tree(S, K, r, q, t, optype='put', steps=50)
            vega_tree2 = Tree(S, K, r, q, t, optype='put', steps=50)
            vega_tree1.params(iv-dV)
            vega_tree2.params(iv+dV)
            vega = (vega_tree2.optionTree() - vega_tree1.optionTree())/(2.0*dV)
            vega /= 100
            Vega.scatter(K, vega, color='red', s=5)

            dR = 0.01
            rho_tree1 = Tree(S, K, r+dR, q, t, optype='put', steps=50)
            rho_tree2 = Tree(S, K, r-dR, q, t, optype='put', steps=50)
            rho_tree1.params(iv)
            rho_tree2.params(iv)
            rho = (rho_tree1.optionTree() - rho_tree2.optionTree())/(2.0*dR)
            rho /= 100
            Rho.scatter(K, rho, color='red', s=5)
